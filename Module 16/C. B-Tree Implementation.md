...
## Aim
To implement a B-Tree with insertion, deletion, searching, and traversal operations.

## Algorithm

1. **Insertion:**
   - If the tree is empty, create a root node.
   - Otherwise, insert the new item recursively in the appropriate node.
   - If a node overflows (exceeds max allowed keys), split the node.
   - Promote the median item to the parent node.

2. **Deletion:**
   - Find the node containing the item.
   - If the node is an internal node, replace the item with its successor.
   - Remove the item from the leaf node.
   - If a node underflows (less than minimum allowed keys), perform rotations or merge with sibling nodes.
   - Adjust the tree accordingly to maintain B-Tree properties.

3. **Searching:**
   - Start at the root.
   - Traverse nodes comparing the key with current node's keys.
   - Move to appropriate child link if not found.
   - Report whether the key is present or not.

4. **Traversal:**
   - Perform an in-order traversal.
   - Recursively visit child nodes and print keys in ascending order.
...
![image](https://github.com/user-attachments/assets/c524a8d0-458a-414e-b890-79c032e16dc3)

## Result
The B-Tree operations of insertion, deletion, searching, and traversal have been implemented successfully.
